/*
   ...
   |  > > CLIPPING (HARD (DISTORTION) / SOFT (OVERDRIVE))     | > > Отсекание (клиппинг) (жёсткий (искажение) / мягкий (перегруз))    |
   |  > > DELAY                                               | > > Задержка                                                          |
   |  > > CHORUS                                              | > > Хор                                                               |
   |  > > FLANGER                                             | > > Флэнжер                                                           |
...
//Время задержки для эффектов ХОР, ФЛЭНЖЕР, ФЕЙЗЕР (для оптимизации указать OPTIMIZATION_DELAY (строка #define OPTIMIZATION_DELAY в программе до подключения заголовочного файла библиотеки), по умолчанию используется стандарт (не работает с параметрами: частота дискретизации 40 кГц, 40 нот, 8 несущих волн (не хватает ресурсов (памяти))!); можно вручную указать время задержки для этих эффектов в парамеире DELAY (строка #define DELAY <время задержки> в программе до подключения заголовочного файла библиотеки))
#ifndef DELAY
#ifdef OPTIMIZATION_DELAY
#define DELAY 10000
#else
#define DELAY 30000
#endif
...
class AudioSynthesizerClass {
  public:
    void enable(uint32_t sampleRate, uint8_t noteCount, uint8_t carrierWaveCount, bool useDAC = USE_ANALOG_OUTPUT, bool mode = MONO, bool useADC = NO_ANALOG_INPUT, bool channel = LEFT_CHANNEL) {            //Разрешить работу синтезатора (частота дискретизации, количество нот, количество несущих волн на одну ноту, использовать ЦАП, использовать АЦП, режим (МОНО / СТЕРЕО), канал (ЛЕВЫЙ / ПРАВЫЙ)) (класс синтезатора очень требователен к ресурсам, в том числе к памяти и времени выполнения, поэтому пользователь может активировать его только по необходимости (в определённом блоке кода))
      _sampleRate = sampleRate;                                 //Частота дискретизации (параметр задаётся в Гц!) (проверено на макс. 40 кГц (40000 Гц))
      ...
      _delay = new Delay(1000000, _sampleRate);                 // |
      _chorus = new Chorus(_sampleRate);                        //  > Эффекты
      _flanger = new Flanger(_sampleRate);                      // |
      _phaser = new Phaser(_sampleRate);                        //_|
      //Устанавливаем флаги
      _useDAC = useDAC;
      _useADC = useADC;
      if (useDAC) {                                             //Если используется аналоговый вsход
        DACInterface.init();                                    //Инициализация ЦАП
        audioOutputMode(mode, channel);                         //Настроить аудио выход
      }
      if (useADC)                                               //Если используется аналоговый вход
        ADCInterface.init();                                    //Инициализация АЦП
      _timeLeft = millis();                                     //Перезапускаем таймер
    }
    ...
    void process() {                                                                                                                                                        //Обработка сигнала
      if (_process and micros() - _timeLeft >= round(1000000 / _sampleRate)) {                                                                                              //Если обработка включена и прошёл ПЕРИОД дискретизации
        ...
        _timeLeft = micros();                                                                           //Перезапускаем таймер, т. к. обработали сигнал и ждём следующий ПЕРИОД дискретизации
      }
    }
    ...
    void volume(uint16_t value) {                     //Громкость
      if (value > 4095)                               //Значение не должно превышать 4095!
        value = 4095;
      _volume = value;
    }
    void output(uint16_t source = DEFAULT_SOURCE) {                                                                     //Вывод аудио (источник (ВНУТРЕННИЙ (INTERNAL_SOURCE) / ВНЕШНИЙ (EXTERNAL_SOURCE) / ПОПОЛАМ (DEFAULT_SOURCE) / <ЗНАЧЕНИЕ от 0 до 4095 (уровень ВНЕШНЕГО источника)>))
      if (_useDAC) {                                                                                                    //Выводить звук можем только если используем (инициализировали) ЦАП!
        uint16_t value = _intValue * map(source, 0, 4095, 1.00, 0.00) + _extValue * map(source, 0, 4095, 0.00, 1.00);   //Значение = внутр. * внутр. коэффициент + внешн. * внешн. коэффициент
        //Применяем параметр громкости (пропорционально меняем амплитуду)
        if (value >= 2048)                                                                                              //Верхняя часть "графика" сигнала (> или = усл. 0 (2048))
          value = map(value, 2048, 4095, 2048, map(_volume, 0, 4095, 2048, 4095));
        else                                                                                                            //Нижняя часть "графика" сигнала (< усл. 0 (2048))
          value = map(value, 2047, 0, 2047, map(_volume, 0, 4095, 2047, 0));
        //Выводим сигнал
